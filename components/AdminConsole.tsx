...
async function parseResponse(response: Response) {
  const payload = await response.json() as { products?: Product[]; orders?: OrderRecord[]; error?: string; publicUrl?: string };
  if (!response.ok) {
...
export function AdminConsole() {
  ...
  const uploadMedia = async () => {
    ...
    const publicUrl = await uploadFileToR2(uploadFile);
    setDraft((state) => ({ ...state, imageUrls: [...(state.imageUrls ?? []), publicUrl].slice(0, 3) }));
    setStatus(`Uploaded product photo: ${publicUrl}`);
    setUploadFile(null);
  };
