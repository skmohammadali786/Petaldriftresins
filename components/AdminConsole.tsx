...
export function AdminConsole() {
  ...
  const [categoryPhotoFiles, setCategoryPhotoFiles] = useState<Record<number, File | null>>({});
  ...
  const saveProduct = async () => {
    const uploadedPhoto = uploadFile ? await uploadFileToR2(uploadFile, 'products') : null;
    const payload = {
      ...draft,
      slug: draft.slug.trim() || toSlug(draft.name),
      imageUrls: uploadedPhoto ? [...(draft.imageUrls ?? []), uploadedPhoto.publicUrl].slice(0, 3) : draft.imageUrls
    };
    ...
    setEditingSlug(null);
    setUploadFile(null);
    setStatus(uploadedPhoto ? `Product saved with uploaded photo (${uploadedPhoto.key}) and live on storefront.` : 'Product saved and live on storefront.');
  };
